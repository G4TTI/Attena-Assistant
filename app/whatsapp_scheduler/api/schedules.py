"""API REST de agendamentos — sempre filtrada pelo dono autenticado."""
...
def _load(db: Session, schedule_id: str, user_id: str) -> Schedule:
...
def list_schedules(
    ...
    stmt = select(Schedule).where(col(Schedule.user_id) == current_user.id).order_by(col(Schedule.created_at).desc())
...
def get_schedule(
    schedule_id: str, db: Session = Depends(get_session), current_user: User = Depends(auth.require_user_api)
) -> ScheduleRead:
    schedule = _load(db, schedule_id, current_user.id)
...
def delete_schedule(
    ...
    if not cancel_schedule(db, schedule_id, user_id=current_user.id):
        raise HTTPException(status_code=404, detail="Agendamento não encontrado.")
...
def trigger_now(
    ...
    dispatch = run_now(db, schedule_id, user_id=current_user.id)
    if dispatch is None:
        raise HTTPException(status_code=404, detail="Agendamento não encontrado.")
